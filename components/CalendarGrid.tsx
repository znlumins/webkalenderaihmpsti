...
import { formatToWIB } from "../lib/dateHelper"; // <--- IMPORT HELPER WIB
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden relative z-0">
      ...
      <div className="grid grid-cols-7 auto-rows-[80px] md:auto-rows-[140px]">
        ...
        {days.map(day => {
          ...
          // Kita tidak pakai isSameDay bawaan karena dia bingung antara UTC/Local
          // Kita ubah kedua tanggal menjadi string "YYYY-MM-DD" dalam zona WIB.
          // Jika stringnya sama, berarti harinya sama.
          
          const dayString = format(day, "yyyy-MM-dd"); // Tanggal kotak kalender

          const dayEvents = events.filter(e => {
             // Ubah waktu event (UTC) menjadi tanggal WIB (yyyy-MM-dd)
             const eventDateString = formatToWIB(e.start_time, "yyyy-MM-dd");
             return dayString === eventDateString;
          });
          // ---------------------------------
                        ...
                        >
                            <div className="overflow-hidden w-full pointer-events-none">
                                <p className="text-[8px] md:text-[10px] font-bold text-gray-800 truncate leading-tight">
                                    {/* Tampilkan Jam dalam WIB juga disini biar konsisten */}
                                    <span className="opacity-70 mr-1 text-[8px]">
                                        {formatToWIB(ev.start_time, "HH.mm")}
                                    </span>
                                    {ev.title}
                                </p>
                            </div>
